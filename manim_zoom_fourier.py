...
class Pi(ZoomedScene):
    def construct(self):
        ...
        ec = FourierEpicyclesMObject(complex_points, num_coefs=1000, speed_factor=0.8,
                                     circles_color=GREEN, circles_opacity=1)

        # Create a zoom window that follows the Fourier epicycles
        zoom_factor = 0.1  # Adjust the zoom scaling factor
        zoom_rect = self.zoomed_camera.frame
        zoom_rect.set_color(RED)
        zoom_rect.set_stroke(width=1)

        # Apply the zoom factor
        zoom_rect.scale(zoom_factor)

        self.zoomed_display.to_corner(DOWN + RIGHT)
